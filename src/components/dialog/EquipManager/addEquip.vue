<template>
  <div class="add-equip" >
    <el-dialog :title="add_dialog_title"
               :visible.sync="dialogFormVisible" v-loading="true">
      <!-- 第一步 start -->
      <el-form :model="form1"
               :rules='rules1'
               ref="dataForm1"
        ...
        <el-form-item label="实验地址："
          ...
          <div class="map">
            <vue-map :parentAddress="form1.address"
              @changeAddress="changeAddress"  v-if="hackReset"></vue-map>
          </div>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 地址组件传过来的地址值
    changeAddress(val) {
      this.form1.address = val
      console.log(this.form1.address)
    },
    // 限制图片上传
          ...
          this.$refs['dataForm1'].validate((valid) => {
            ...
            if (valid) {
              ...
              if (this.form1.id === '') {
                ...
                // 判断角色
                editEquipment(this.form1).then(res => {
                  let data = res.data
                  if (data.success) {
                    if (!status) {
                      this.step++
    ...
    initForm (val) {
      ...
      this.form2.ismycreate = val.instrumentin.ismycreate
      this.form2.iscustomssupervise = val.instrumentin.iscustomssupervise
