<template>
  <div class="unitUser zx-flex column">
      ...
      <div class="select-item">
        <div class="operation-button zx-flex align">
          <el-button type="primary"
                     ...
                     @click.stop.prevent="">批量导入</el-button>
        </div>
      </div>
    </div>
    <!-- 列表 -->
    ...
    <create-category :showDialog="showcCreateDialog"
                     @returnBool=" bool => { showcCreateDialog = bool }"></create-category>
    <service-category-detail :showDialog="showcDetailDialog"
                             @returnBool=" bool => { showcDetailDialog = bool }"></service-category-detail>
  </div>
</template>
...
// api
import { serviceCategory, unitUserDetail, unitUserAudit } from '@/api/clientManage'
// 数据字典
import { getDataDictionary } from '@/api'
export default {
  name: 'unitUser',
  components: { createCategory, serviceCategoryDetail },
  data () {
    return {
  ...
  methods: {
    ...
    initTableData () {
      ...
      serviceCategory(query).then(res => {
        console.log(res)
