...
import { getContarct, getOrderList, getBillList, getCostList, getEvaluateList } from '@/api/order'
export default {
  name: 'orderDetail',
  data () {
    return {
  ...
  created () {
    ...
    // console.log('this.bbaseInfobaseInfoaseIngo', this.baseInfo)
  },
  methods: {
    ...
    getOrderAddress () {
      getOrderList('order001').then(res => {
        if (res.data.success) {
          this.ShowDisposeDialog = true // 获取数据成功显示
          console.log('res.datad', res.data)
    ...
    getOrderEvaluate () {
      getEvaluateList('order001').then(res => {
        if (res.data.success) {
          this.ShowDisposeDialog = true // 获取数据成功显示
