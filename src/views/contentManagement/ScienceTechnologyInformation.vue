      ...
      <div class="service-header zx-flex between">
        ...
        <div class="operation-button">
          <el-button type="primary"
                     ...
                     @click="Checkinformation(false,'新增-科技资讯',{})">新增</el-button>
          <el-button type="primary"
        ...
        <el-table :data="tableData"
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button type="text"
                         size="small"
                         @click="Checkinformation(true,'详情-科技资讯',scope.row)">查看</el-button>
              <el-button type="text"
                         size="small"
                         @click="Checkinformation(false,'编辑-科技资讯',scope.row)">编辑</el-button>
              <el-button type="text"
                         size="small"
...
import { queryScienceInfoList, pubScienceInfo, topScienceInfo, delScienceInfo } from '@/api/contentManagementApi'
export default {
  components: {
    contentPopup
  },
  ...
  methods: {
    ...
    queryScienceInfoListGM () {
      this.loading = true
      queryScienceInfoList(this.formList).then(res => {
        if (res.data.success) {
    ...
    // 批量上下架通用
    pubScienceInfoGM (qurey) {
      this.loading = true
      pubScienceInfo(qurey).then(res => {
        if (res.data.success) {
    ...
    deleteS (id) {
      ...
      delScienceInfo(idsOBJ).then(res => {
        if (res.data.success) {
