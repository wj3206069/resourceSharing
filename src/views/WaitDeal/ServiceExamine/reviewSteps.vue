      ...
      <div class="popping_s basic_services">
        <el-form :model="new_service"
          ...
          <el-form-item label="自定义周期:"
                        v-if="this.new_service.period==='3'">
          ...
          <el-form-item label="选择委托单:"
            ...
            <el-select v-model="new_service.orderticket"
                       placeholder="请选择"
                       :disabled='detailsDisabled'>
              <el-option label="委托单1"
                         value="0"></el-option>
              <el-option label="委托单2"
                         value="1"></el-option>
            </el-select>
            <span class="preview"
...
<script>
// import './rules'
import { addRelatedQualification, addFacilitatingAgency, addSharingType, addServiceType, addCancelAssociated, getServiceInformationGM, editServiceInformationGM, getDetectionServiceInformation, associationCancellationDetectionServiceInformation, EquiClassification, getServicePriceInformation, deleteFile, getBankaccountPublicaccounts, getServiceaccounts } from '@/api/serviceManagerPlatformApi'
      ...
      if (this.title === '编辑' || this.title === '详情') {
        ...
        getServiceInformationGM({ business_id: this.id_s, editor: 1 }).then(res => {
          if (res.data.success) {
            ...
            console.log(res.data.serviceInformation)
            ...
            for (let i = 0; i < picIds.length; i++) {
              this.fileList.push({ url: `http://127.0.0.1:8088/Sz_dxyq/fileAction/viewFile/${picIds[i]}` })
            }
            let picIdsS = res.data.serviceInformation.details_ids.split(',')
            for (let i = 0; i < picIdsS.length; i++) {
              this.fileListS.push({ url: `http://127.0.0.1:8088/Sz_dxyq/fileAction/viewFile/${picIdsS[i]}` })
            }
            // }
            // 第五步赋值
  ...
  methods: {
    ...
    accountDetermine () {
      for (let i = 0; i < this.multipleSelection.length; i++) {
        ...
        this.SelectaccountList.push(this.multipleSelection[i])
      }
